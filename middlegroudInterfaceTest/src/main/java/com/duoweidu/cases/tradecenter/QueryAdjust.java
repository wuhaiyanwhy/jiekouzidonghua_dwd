...
public class QueryAdjust extends TradeCenterInterfaceTest {
    ...
    @Test(dependsOnGroups = "iqgCreateAdjust", description = "查询调账单")
    public void iqgQueryAdjust() {
        List<NameValuePair> list = new LinkedList<>();
        list.add(new BasicNameValuePair("method", "trade.query_adjust"));
    ...
    private void iqgDetailAssert() {
        ...
        detailAssertTest(SqlDetail.getInstance().getParamValue(0, "adjustDesc"), "adjust_desc", model.adjust_desc);
        detailAssertTest("origin_data", model.origin_data.toString());
        detailAssertTest(TradecenterConfig.iqgCreateAdjustTradeNo, "origin_data.trade_no", model.origin_data.trade_no);
